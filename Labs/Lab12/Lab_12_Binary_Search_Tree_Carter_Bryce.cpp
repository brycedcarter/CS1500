...
void insert(Employee* node, Employee** root)
{
	...
	int compareValue = (*root)->getFirstName().compare(node->getFirstName());
	if (compareValue > 0)
	{
	...
	{
		cout << "\nERROR!\n The employee is already in the tree and therefore cannot be inserted.\nInsert Fialed!\n\n";
	}
}
...
Employee* depthFirstSearch(string firstName, Employee* root)
{
	...
	int compareValue = root->getFirstName().compare(firstName);
